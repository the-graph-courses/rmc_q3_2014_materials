...
1.  **People living with HIV:** This dataset contains the total number of people currently infected with HIV (*data/raw/people_living_with_hiv_number_all_ages.csv*).
2.  **New HIV infections:** This dataset provides the total number of people newly infected with HIV during a given year (*data/raw/newly_hiv_infected_number_all_ages.csv*).
...
**HINT:** Ensure that you are joining on `country`, `country_iso` and `year` to accurately align the datasets.
...
**CHECKPOINT:** The `final_dataset` should now include population data alongside the HIV case data, allowing for more detailed analysis. You should have a dataset with **3278 rows and 8 columns.**

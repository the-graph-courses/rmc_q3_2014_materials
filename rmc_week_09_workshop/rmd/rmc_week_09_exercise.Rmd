...
## Step 2.2: Import Attribute Data | Malaria Cases
...
**TASK:** Selecting the columns containing the state names, the columns containing the cases, and the column containing the geometry from `nga_malaria`. Rename the `name_1` with `state_name`
...
- Use a `for` loop to iterate through each year and filter the dataset for that year.
- Ensure that the color scale for malaria cases remains consistent across all maps by setting the scale limits using the `scale_fill_viridis_c()` function. 
